...
function getDatabaseUrl() {
  ...
  if (!databaseUrl) {
    console.error('Missing database URL. Set RAILWAY_DATABASE_URL (preferred) or DATABASE_URL in backend/.env.');
    process.exit(1);
  }
  return databaseUrl;
}
...
const fileName = `railway_backup_${formatTimestamp(new Date())}.dump`;
const localOutputPath = path.join(backupDir, fileName);

const baseDumpArgs = ['--no-owner', '--no-privileges', '--format=custom'];
...
if (hasCommand('pg_dump')) {
  ...
  console.log('Backup created with pg_dump.');
  process.exit(0);
}
...
if (hasCommand('docker')) {
  ...
  console.log('Backup created with docker postgres:16 image.');
  process.exit(0);
}
