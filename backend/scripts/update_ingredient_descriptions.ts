...
const IN_FILE = path.resolve(__dirname, '..', '..', 'ingredient_descriptions.generated.json');

async function main() {
  const raw = await fs.readFile(IN_FILE, 'utf8');
  const payload = JSON.parse(raw) as DescriptionInput[];
  ...
  console.log('Ingredient description update report');
