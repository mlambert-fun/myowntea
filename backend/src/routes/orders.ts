...
const mapBoxtalStatus = (status?: string | null) => {
    ...
    if (['CREATED', 'CONFIRMED', 'REGISTERED'].includes(normalized))
        return 'CREATED';
    if (['DOCUMENT_CREATED', 'LABEL_CREATED', 'READY'].includes(normalized))
        return 'LABEL_CREATED';
    if (['IN_TRANSIT', 'PICKED_UP', 'SHIPPED', 'IN_DELIVERY'].includes(normalized))
        return 'IN_TRANSIT';
        ...
        if (shippingSelection?.offerCode) {
            try {
                const shipper = {
                    ...
                    address: {
                        line1: process.env.BOXTAL_SHIPPER_ADDRESS1 || t("backend.routes.orders.address_expediteur"),
                ...
                const recipient = {
                    ...
                    address: {
                        line1: user.customer.address || t("backend.routes.orders.address_indisponible"),
                        line2: '',
