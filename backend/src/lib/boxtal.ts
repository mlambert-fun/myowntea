...
function sanitizeBoxtalLogValue(value: unknown, parentKey = ''): unknown {
    ...
    if (typeof value === 'string') {
        if (key.includes(t("backend.lib.boxtal.email")))
            return maybeMaskEmail(value);
        if (key.includes('phone') || key.includes('dialcode') || key.includes('number'))
            return maybeMaskPhone(value);
...
async function boxtalRequest<T>({ method = 'GET', path, query, body, responseType = 'json', }: BoxtalRequestOptions): Promise<T> {
    ...
    try {
        ...
        throw new Error(`Boxtal API parse error (${response.status}): ${error instanceof Error ? error.message : String(error)}`);
    }
}
...
function parseLegacyQuoteOffers(xml: string): LegacyQuoteOffer[] {
    ...
    return blocks.map((block) => {
        ...
        const explicitOfferCode = extractXmlTagPath(block, ['shippingOfferCode']) ||
        ...
        const offerId = extractXmlTagPath(block, ['shippingOfferId']) ||
...
function isRelayOffer(offer: LegacyQuoteOffer) {
    const relaySignals = [
        ...
        .join(' ');
    return (relaySignals.includes('PICKUP_POINT') ||
...
function selectLegacyQuoteOffer(params: {
    ...
    const byMode = params.mode === 'RELAY'
...
export async function quoteShippingOffer(params: {
    ...
    const query = {
        ...
        'recipient.adresse': toNonEmptyStringOrNull(params.addressLine1) || t("backend.lib.boxtal.address_destinataire"),
    ...
    const selectedOfferId = toNonEmptyStringOrNull(selected?.offerId) ||
