...
export const FIRST_ORDER_ONLY_NOT_APPLIED_MESSAGE = t("backend.lib.discounts.code_non_applique_8");
...
const isEligibleBase = (discount: DiscountCandidate, subtotalCents: number, now: Date) => {
    ...
    if (!isWithinDateRange(discount, now))
        return false;
    if ((discount.minimumSubtotalCents || 0) > subtotalCents)
        return false;
    return true;
};
const normalizeItems = (items: DiscountCartItem[] | undefined): NormalizedItem[] => {
    ...
    return items
        ...
        return {
            ...
            subscriptionPlanId: typeof item.subscriptionPlanId === 'string' && item.subscriptionPlanId.trim().length > 0
...
const matchesTarget = (item: NormalizedItem, target: TargetConfig) => {
    if (!target.includeGiftItems && item.isGift)
        return false;
    if (target.itemTypes.size > 0 && !target.itemTypes.has(item.itemType))
        return false;
    if (target.excludeItemTypes.size > 0 && target.excludeItemTypes.has(item.itemType))
        return false;
    if (target.productIds.size > 0 && (!item.productId || !target.productIds.has(item.productId)))
        return false;
    if (target.variantIds.size > 0 && (!item.variantId || !target.variantIds.has(item.variantId)))
        return false;
    if (target.subscriptionPlanIds.size > 0 &&
...
const resolveScopeBase = (discount: DiscountCandidate, subtotalCents: number, productSubtotalCents: number, shippingCents: number) => {
...
const computeStandardDiscountAmountCents = (discount: DiscountCandidate, subtotalCents: number, productSubtotalCents: number, shippingCents: number) => {
...
const parseTierRules = (discount: DiscountCandidate): TierRule[] => {
    ...
    if (fallbackPercent <= 0 && fallbackFixed <= 0)
        return [];
    return [
        {
...
const computeTieredDiscountCents = (discount: DiscountCandidate, items: NormalizedItem[]) => {
    ...
    let bestAmount = 0;
    tiers.forEach((tier) => {
        if (eligibleQty < tier.minQty)
            return;
        if (eligibleSubtotalCents < tier.minSubtotalCents)
            return;
...
const computeSubscriptionDiscountCents = (discount: DiscountCandidate, items: NormalizedItem[]) => {
    ...
    if (fixedOffCents > 0) {
        const fixedAmount = eligible.reduce((sum, item) => sum + Math.min(item.unitPriceCents, fixedOffCents) * item.quantity, 0);
        return Math.min(eligibleSubtotalCents, fixedAmount);
    }
    return 0;
};
const computeGiftDiscountCents = (discount: DiscountCandidate, items: NormalizedItem[], subtotalCents: number) => {
    const config = toObject(discount.config);
    ...
    const threshold = Math.max(toInt(config.triggerMinimumSubtotalCents, 0), toInt(discount.minimumSubtotalCents, 0));
...
const computeDiscountAmountCents = (discount: DiscountCandidate, subtotalCents: number, productSubtotalCents: number, shippingCents: number, items: NormalizedItem[]) => {
    switch (discount.type) {
        case 'PERCENTAGE':
        case 'FIXED':
        case 'FREE_SHIPPING':
...
const buildCodeNotAppliedReason = (params: {
    ...
    const codeCandidates = params.discounts.filter((discount) => discount.method === 'CODE' && normalizeCode(discount.code) === params.code);
    if (codeCandidates.length === 0) {
        return t("backend.lib.discounts.code_non_applique_5");
    }
    const activeCandidates = codeCandidates.filter((discount) => discount.status === 'ACTIVE');
    if (activeCandidates.length === 0) {
        return t("backend.lib.discounts.code_non_applique_4");
    }
    ...
    if (dateEligibleCandidates.length === 0) {
        return t("backend.lib.discounts.code_non_applique_3");
    }
    const subtotalEligibleCandidates = dateEligibleCandidates.filter((discount) => (discount.minimumSubtotalCents || 0) <= params.subtotalCents);
    if (subtotalEligibleCandidates.length === 0) {
        const requiredMinSubtotalCents = Math.min(...dateEligibleCandidates.map((discount) => discount.minimumSubtotalCents || 0));
        return `Code non appliqué: ce code est valable à partir de ${formatCentsAsEuro(requiredMinSubtotalCents)} &euro;.`;
    }
    const firstOrderEligibleCandidates = subtotalEligibleCandidates.filter((discount) => !Boolean(discount.firstOrderOnly) || params.isFirstOrderEligible);
    if (firstOrderEligibleCandidates.length === 0) {
        ...
        return t("backend.lib.discounts.code_non_applique_7");
    }
    ...
    if (totalUsageEligibleCandidates.length === 0) {
        return t("backend.lib.discounts.code_non_applique_2");
    }
    ...
    if (perCustomerEligibleCandidates.length === 0) {
        if (!params.customerEmail &&
            totalUsageEligibleCandidates.some((discount) => discount.usageLimitPerCustomer !== null &&
                discount.usageLimitPerCustomer !== undefined)) {
            return t("backend.lib.discounts.code_non_applique_9");
        }
        return t("backend.lib.discounts.code_non_applique");
    }
    return t("backend.lib.discounts.code_non_applique_7");
};
export function computeDiscounts(input: DiscountComputationInput): DiscountComputationResult {
    ...
    const eligibleWithFirstOrderConstraint = eligibleBase.filter((discount) => !Boolean(discount.firstOrderOnly) || isFirstOrderEligible);
    const eligibleWithUsage = eligibleWithFirstOrderConstraint.filter((discount) => {
        ...
        if (discount.usageLimitTotal !== null && discount.usageLimitTotal !== undefined) {
            if (usage.totalRedemptions >= discount.usageLimitTotal)
                return false;
        }
        if (discount.usageLimitPerCustomer !== null && discount.usageLimitPerCustomer !== undefined) {
    ...
    if (normalizedAppliedCode) {
        const matching = eligibleWithUsage.filter((discount) => discount.method === 'CODE' && normalizeCode(discount.code) === normalizedAppliedCode);
        if (matching.length === 0) {
            messages.push(buildCodeNotAppliedReason({
    ...
    const discountWithAmounts = eligible
        .map((discount) => ({
        discount,
        amountCents: computeDiscountAmountCents(discount, input.subtotalCents, productSubtotalCents, input.shippingCents, normalizedItems),
    }))
    ...
    if (normalizedAppliedCode) {
        const codeEntries = discountWithAmounts.filter((entry) => entry.discount.method === 'CODE' &&
            normalizeCode(entry.discount.code) === normalizedAppliedCode);
        const bestCodeEntry = codeEntries.reduce<{
            discount: DiscountCandidate;
            amountCents: number;
        } | null>((best, current) => {
            if (!best || current.amountCents > best.amountCents)
                return current;
            return best;
        }, null);
        if (bestCodeEntry) {
            ...
            const stackableAutomatic = discountWithAmounts.filter((entry) => entry.discount.method === 'AUTOMATIC' && entry.discount.stackable);
    ...
    if (applied.length === 0) {
        ...
        const bestSingle = discountWithAmounts.reduce<{
            discount: DiscountCandidate;
            amountCents: number;
        } | null>((best, current) => {
            if (!best || current.amountCents > best.amountCents)
    ...
    const codeWasApplied = Boolean(normalizedAppliedCode &&
        applied.some((entry) => entry.discount.method === 'CODE' && normalizeCode(entry.discount.code) === normalizedAppliedCode));
    if (normalizedAppliedCode && codeDiscounts.length > 0 && !codeWasApplied) {
        messages.push(t("backend.lib.discounts.code_non_applique_6"));
    }
    ...
    const discountTotalCents = Math.min(cappedSubtotalDiscount + cappedShippingDiscount, input.subtotalCents + input.shippingCents);
