...
export default function ProductForm() {
  ...
  async function handleSaveProduct(e: React.FormEvent) {
    ...
    try {
      ...
      if (!formData.title.trim() || !formData.slug.trim()) {
        setProductError('Le titre et le slug sont requis.');
        return;
      }
  ...
  async function handleProductImageUpload(file?: File | null) {
    ...
    try {
      ...
      try {
        ...
        setProductImageError('Upload impossible. Reessayez ou changez de fichier.');
      }
    }
  }
  ...
  async function handleDeleteOption(optionId: string) {
    ...
    try {
      ...
      if (product?.id) await loadOptions(product.id);
    } catch (error) {
  ...
  async function handleSaveVariant(e: React.FormEvent) {
    ...
    try {
      ...
      if (!variantForm.priceCents || Number.isNaN(Number(variantForm.priceCents))) {
        setVariantError('Le prix est requis.');
        return;
      }
      ...
      if (optionsWithValues.length > 0) {
        ...
        for (const option of optionsWithValues) {
          ...
          if (!found) {
            setVariantError(`S?lectionnez une valeur pour l?option "${option.name}".`);
            return;
          }
  ...
  function resetVariantForm() {
    ...
    setVariantError(null);
    setVariantForm({ sku: '', priceCents: '', stockQty: '', imageUrl: '', isActive: true, optionValueIds: [] });
  }
  ...
  async function handleDeleteVariant(variantId: string) {
    ...
    try {
      ...
      if (product?.id) await loadVariants(product.id);
    } catch (error) {
  ...
  async function handleVariantImageUpload(file?: File | null) {
    ...
    try {
      const optimized = await optimizeImageFile(file);
      const response = await api.uploadImage(optimized, folder, token);
      setVariantForm((prev) => ({ ...prev, imageUrl: response.url }));
    } catch (error) {
      ...
      try {
        ...
        setVariantImageError('Upload impossible. Reessayez ou changez de fichier.');
      }
    }
  }
  ...
  const expectedVariantCount = useMemo(() => {
    if (optionsWithValues.length === 0) return 0;
    ...
    <Layout>
      <div className="admin-page admin-page-premium-lite">
        <div className="admin-header">
          <div>
            <h1 className="admin-title">{isNew ? 'Nouveau produit' : 'Modifier le produit'}</h1>
            <p className="admin-subtitle">Gestion des options et variantes</p>
          </div>
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            <Link className="admin-btn" to="/products">
              Retour ? la liste
            </Link>
          </div>
        </div>
          ...
          <>
            <div className="admin-card">
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                {productImageError && (
                  <div className="admin-alert" style={{ marginTop: '0.5rem' }}>
                    {productImageError}
                  </div>
                )}
              </div>
              ...
              <>
                ...
                {!hasOptions && (
                  <p className="admin-muted" style={{ marginBottom: '1rem' }}>
                    Ajoutez des options/valeurs pour cr?er des variantes.
                  </p>
                )}
                {incompleteOptions.length > 0 && (
                  <div className="admin-alert" style={{ marginBottom: '1rem' }}>
                    Certaines options n?ont pas encore de valeurs :{' '}
                    {incompleteOptions.map((opt) => opt.name).join(', ')}.
                  </div>
                )}
                  {showVariantForm && (
                    <form onSubmit={handleSaveVariant} className="admin-form-grid">
                      <div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setVariantForm({ ...variantForm, priceCents: e.target.value })}
                          required
                        />
                      </div>
                      <div>
                          ...
                          {options.map((option) => (
                            <div key={option.id} className="admin-card" style={{ background: '#fff' }}>
                              ...
                              <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '0.75rem' }}>
                                {option.values.map((value) => (
                                  <label key={value.id} className="admin-chip">
                                    <input
                                      type="checkbox"
                                      checked={variantForm.optionValueIds.includes(value.id)}
                      ...
                      <div style={{ display: 'flex', gap: '0.75rem' }}>
                        <button className="admin-btn admin-btn-primary" type="submit">
                          {editingVariantId ? 'Mettre ? jour' : 'Cr?er'}
                        </button>
                        {editingVariantId && (
                          <button className="admin-btn" type="button" onClick={resetVariantForm}>
                            Annuler
                          </button>
                        )}
                      </div>
                      {variantError && <div className="admin-alert">{variantError}</div>}
                      {hasOptions && optionsWithValues.length > 0 && (
                        <div className="admin-muted" style={{ gridColumn: '1 / -1' }}>
                          S?lectionnez une valeur par option.
                        </div>
                      )}
                    </form>
                  )}
                      ...
                      {variants.map((variant) => (
                        <tr key={variant.id}>
                          ...
                          <td style={{ display: 'flex', gap: '0.5rem' }}>
                            <button className="admin-btn admin-btn-secondary" onClick={() => startEditVariant(variant)}>
                              Modifier
                            </button>
                ...
                {expectedVariantCount > 0 && variants.length < expectedVariantCount && (
                  <div className="admin-alert" style={{ marginTop: '1rem' }}>
                    Combinaisons incompl?tes : {variants.length} / {expectedVariantCount} variantes cr??es.
                  </div>
                )}
                </div>
              </>
