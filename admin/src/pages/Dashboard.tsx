...
export default function Dashboard() {
  ...
  async function loadDashboard() {
    ...
    if (ingredientsResult.status === 'fulfilled' && Array.isArray(ingredientsResult.value)) {
      ...
      setIngredients([]);
      if (ingredientsResult.status === 'rejected') loadErrors.push('ingrédients');
    }
    ...
    if (ordersResult.status === 'fulfilled' && Array.isArray(ordersResult.value)) {
      ...
      setOrders([]);
      if (ordersResult.status === 'rejected') loadErrors.push('commandes');
    }
    ...
    if (shipmentsResult.status === 'fulfilled' && Array.isArray(shipmentsResult.value)) {
      ...
      setShipments([]);
      if (shipmentsResult.status === 'rejected') loadErrors.push('expéditions');
    }
    ...
    if (jobsResult.status === 'fulfilled' && Array.isArray(jobsResult.value)) {
      ...
      setJobs([]);
      if (jobsResult.status === 'rejected') loadErrors.push('jobs automatiques');
    }
    ...
    if (customersResult.status === 'fulfilled' && Array.isArray(customersResult.value)) {
      ...
      setCustomers([]);
      if (customersResult.status === 'rejected') loadErrors.push('clients');
    }
    ...
    if (cartsResult.status === 'fulfilled' && Array.isArray(cartsResult.value)) {
      ...
      setCarts([]);
      if (cartsResult.status === 'rejected') loadErrors.push('paniers');
    }
  ...
  const dashboard = useMemo(() => {
    ...
    return {
      ...
      ordersToProcess:
        (statusCounts.PENDING || 0) + (statusCounts.CONFIRMED || 0) + (statusCounts.PROCESSING || 0),
      ...
      <div className="admin-page admin-dashboard">
        <section className="admin-card dashboard-hero">
          <div>
            ...
            <p className="admin-subtitle">
              Vue pilotage: commandes, expéditions, communication client et points de blocage.
            </p>
            <p className="dashboard-meta">Dernière actualisation: {formatDateTime(lastUpdatedAt)}</p>
          </div>
          <button
        ...
        <section className="dashboard-kpi-grid">
          <article className="dashboard-kpi-card">
            <p className="dashboard-kpi-label">CA du mois</p>
          ...
          <article className="dashboard-kpi-card dashboard-kpi-warning">
            <p className="dashboard-kpi-label">À traiter</p>
          ...
          <article className="dashboard-kpi-card">
            <p className="dashboard-kpi-label">Panier moyen</p>
            <p className="dashboard-kpi-value">{CURRENCY_FORMATTER.format(dashboard.averageBasket)}</p>
            <p className="dashboard-kpi-meta">sur commandes payées</p>
          </article>
          <article className="dashboard-kpi-card">
          ...
          <article className="dashboard-kpi-card">
            <p className="dashboard-kpi-label">Ingrédients actifs</p>
          ...
          <article className="dashboard-kpi-card dashboard-kpi-info">
            <p className="dashboard-kpi-label">Expéditions en transit</p>
            <p className="dashboard-kpi-value">{NUMBER_FORMATTER.format(dashboard.shipmentsInTransit)}</p>
            <p className="dashboard-kpi-meta">{NUMBER_FORMATTER.format(dashboard.shipmentsWithoutTracking)} sans suivi</p>
          </article>
          <article className="dashboard-kpi-card dashboard-kpi-danger">
            <p className="dashboard-kpi-label">Alertes opérationnelles</p>
        ...
        <section className="dashboard-grid">
          <article className="admin-card dashboard-panel dashboard-panel-chart">
            <h2 className="admin-card-title">Priorités du jour</h2>
            ...
            <div className="dashboard-actions">
              <Link to="/orders" className="admin-btn admin-btn-primary">
                Voir les commandes
              </Link>
              <Link to="/shipments" className="admin-btn admin-btn-secondary">
                Voir les expéditions
              </Link>
              <Link to="/automation-jobs" className="admin-btn admin-btn-secondary">
                Voir les jobs
              </Link>
              <Link to="/emails" className="admin-btn admin-btn-secondary">
                Voir les emails
              </Link>
            </div>
          </article>
          ...
          <article className="admin-card dashboard-panel">
            ...
            <div className="dashboard-status-grid">
              {ORDER_STATUS_PRIORITY.map((statusCode) => (
                <div key={statusCode} className="dashboard-status-card">
                  <span className="dashboard-status-label">
                    <span
                      className="dashboard-status-dot"
                      style={{ backgroundColor: ORDER_STATUS_COLORS[statusCode] || '#64748b' }}
                    />
        ...
        <section className="dashboard-grid dashboard-grid-large">
          <article className="admin-card dashboard-panel">
            <h2 className="admin-card-title">Activité des 7 derniers jours</h2>
            <div className="dashboard-chart-wrap">
              <svg
                className="dashboard-chart"
                viewBox={`0 0 ${chartConfig.width} ${chartConfig.height}`}
                role="img"
                aria-label="Graphique à barres des commandes sur les 7 derniers jours"
              >
                <defs>
                ...
                {chartConfig.ticks.map((tickValue) => {
                  const y =
                    chartConfig.marginTop + chartConfig.plotHeight - (tickValue / chartConfig.yMax) * chartConfig.plotHeight;
                  return (
                    <g key={tickValue}>
                      <line
                        x1={chartConfig.marginLeft}
                        y1={y}
                        x2={chartConfig.marginLeft + chartConfig.plotWidth}
                        y2={y}
                        stroke="rgba(65, 76, 22, 0.14)"
                        strokeWidth="1"
                      />
                ...
                {chartConfig.bars.map((bar) => (
                  <g key={bar.key}>
                    ...
                    <text
                      x={bar.centerX}
                      y={chartConfig.marginTop + chartConfig.plotHeight + 22}
                      textAnchor="middle"
                      className="dashboard-chart-xlabel"
                    >
                      {bar.label}
                    </text>
                  </g>
                  ...
                  {dashboard.recentOrders.map((order) => {
                    ...
                    return (
                      <tr key={order.id}>
                        ...
                        <td>{customerName || order.customer?.email || 'Invité'}</td>
                        <td>{CURRENCY_FORMATTER.format(resolveOrderAmount(order))}</td>
                        <td>
                          <span
                            className="admin-badge"
                            style={{ backgroundColor: ORDER_STATUS_COLORS[status] || '#64748b', color: '#fff' }}
                          >
                            {label}
                          </span>
                        </td>
        ...
        <section className="admin-card dashboard-panel">
          <h2 className="admin-card-title">Santé des automatisations</h2>
          <div className="dashboard-health-grid">
            ...
            <div className="dashboard-health-item">
              <p className="dashboard-health-label">Emails en échec</p>
