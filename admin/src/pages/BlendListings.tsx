...
const toDraft = (listing?: BlendListing | null): ListingDraft => {
    const ingredientIds = Array.from(new Set((listing?.blend?.ingredients || [])
...
const parseIngredientIds = (value: string) => value
    .split('|')
    .map((id) => id.trim())
    .filter(Boolean);
export default function BlendListings() {
    ...
    const ingredientOptions = useMemo(() => ingredients.slice().sort((a, b) => a.name.localeCompare(b.name, 'fr')), [ingredients]);
    const nextCreateRanking = useMemo(() => listings.reduce((maxRanking, listing) => {
    ...
    async function loadData() {
        ...
        try {
            ...
            setIngredients(Array.isArray(ingredientData)
                ...
                : []);
            setOrders(Array.isArray(orderData)
                ? orderData
    ...
    const validateDraft = (draft: ListingDraft) => {
        ...
        if (!draft.title.trim()) {
            validationErrors.title = t("admin.pages.blend_listings.titre_blend_obligatoire");
        }
        if (!draft.blendName.trim()) {
            validationErrors.blendName = t("admin.pages.blend_listings.last_name_blend_obligatoire");
        }
        if (!draft.ingredientIds.length) {
            validationErrors.ingredientIds = t("admin.pages.blend_listings.add_ingredient_blend");
        }
    ...
    async function saveEditedListing() {
        ...
        try {
            ...
            const message = t("admin.pages.blend_listings.failed_save_blend");
    ...
    async function createListing() {
        ...
        try {
            ...
            const message = t("admin.pages.blend_listings.failed_create_blend");
            setEditorError(message);
            showToast(message, 'error');
        }
        finally {
    ...
    async function deleteListing(listing: BlendListing) {
        ...
        try {
            ...
            const message = t("admin.pages.blend_listings.failed_delete_blend");
            setEditorError(message);
            showToast(message, 'error');
        }
        finally {
    ...
    const getListingIngredientIds = (listing: BlendListing) => Array.from(new Set((listing.blend?.ingredients || [])
        .map((entry) => entry.ingredientId || entry.ingredient?.id)
        .filter((id): id is string => Boolean(id))));
    ...
    async function importCsvFile() {
        if (!csvFile) {
            setCsvImportError(t("admin.pages.blend_listings.select_file_csv"));
            setCsvImportSummary(null);
            return;
        }
        try {
            ...
            if (rows.length === 0) {
                setCsvImportError(t("admin.pages.blend_listings.file_csv_empty"));
                return;
            }
            ...
            for (let rowIndex = 1; rowIndex < rows.length; rowIndex += 1) {
                ...
                const getCell = (header: (typeof CSV_HEADERS)[number]) => String(row[headerIndex[header]] ?? '').trim();
                ...
                if (!draft.title || !draft.blendName || draft.ingredientIds.length === 0) {
                    failures.push(`Ligne ${lineNumber}: titre, nom du blend et ingredientIds sont obligatoires.`);
                    continue;
                }
                try {
            ...
            if (failures.length === 0) {
                ...
                setCsvImportSummary(`${successCount} blend(s) importé(s), ${failures.length} erreur(s).\n${preview}${suffix}`);
            }
        }
        catch (error) {
    ...
    const renderIngredientSelect = (value: string[], onChange: (next: string[]) => void) => (<select multiple className="admin-input" value={value} onChange={(e) => {
            const selected = Array.from(e.target.selectedOptions, (option) => option.value);
            onChange(selected);
        }} style={{ minWidth: 240, height: 96 }}>
      {ingredientOptions.map((ing) => (<option key={ing.id} value={ing.id}>
    ...
    return (<Layout>
      <div className="admin-page admin-page-premium-lite">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', marginBottom: '1rem' }}>
          <div>
            <h2>{t("admin.pages.blend_listings.blends_blend_listings")}</h2>
            <p className="admin-muted">{t("admin.pages.blend_listings.create_edit_blends")}</p>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
            ...
            {!createDraft && (<button className="admin-btn admin-btn-primary" onClick={openCreateForm}>
                Nouveau blend
              </button>)}
          </div>
        </div>

        {editorError && (<div className="admin-alert admin-alert-error" role="alert">
            {editorError}
          </div>)}

        {createDraft && (<div className="admin-card" style={{ marginBottom: '1rem' }}>
            <h3 className="admin-card-title" style={{ marginBottom: '1rem' }}>
              {editingId ? t("admin.pages.blend_listings.edit_blend_listing") : t("admin.pages.blend_listings.new_blend_listing")}
            </h3>
            <div className="admin-grid-2">
              <div>
                <label className="admin-label">Ranking</label>
                <input className="admin-input" type="number" min={0} value={createDraft.ranking} onChange={(e) => updateCreateDraft((d) => ({
                ...d,
              ...
              <div>
                ...
                <input className="admin-input" value={createDraft.title} onChange={(e) => {
              ...
              <div>
                ...
                <input className="admin-input" value={createDraft.slug} onChange={(e) => {
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <textarea className="admin-input" rows={3} value={createDraft.description} onChange={(e) => updateCreateDraft((d) => ({ ...d, description: e.target.value }))} placeholder={t("admin.pages.blend_listings.description_courte_blend")}/>
              </div>
              <div style={{ gridColumn: '1 / -1', display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: '0.75rem', alignItems: 'end' }}>
                <div>
                  <label className="admin-label">{t("admin.pages.blend_listings.order_source_id")}</label>
                  <select className="admin-input" value={createDraft.createdFromOrderId} onChange={(e) => updateCreateDraft((d) => ({ ...d, createdFromOrderId: e.target.value }))}>
                    ...
                    {orders.map((order) => (<option key={order.id} value={order.id}>{order.orderNumber || order.id}</option>))}
                  </select>
                </div>
                <div className="admin-muted" style={{ paddingBottom: '0.5rem' }}>{t("admin.pages.blend_listings.or")}</div>
                <div>
                  <label className="admin-label">{t("admin.pages.blend_listings.created_createdby")}</label>
              ...
              <div>
                ...
                <input className="admin-input" value={createDraft.blendName} onChange={(e) => {
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                <label className="admin-label">{t("admin.pages.blend_listings.ingredient_ids")}</label>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem', marginTop: '1rem' }}>
              <button className="admin-btn admin-btn-secondary" onClick={closeEditor}>
                Annuler
              </button>
              <button type="button" className="admin-icon-button admin-btn-secondary" aria-label="Enregistrer" title="Enregistrer" onClick={editingId ? saveEditedListing : createListing} disabled={Boolean(savingId)}>
        ...
        {loading ? (<p>{t("admin.pages.translations.loading")}</p>) : listings.length === 0 ? (<p>{t("admin.pages.blend_listings.none_blend_available")}</p>) : (<div className="admin-card">
              ...
              <table className="admin-table">
                <thead>
                  <tr>
                    ...
                    <th>Cover blend</th>
                    <th>Ingredients</th>
                    <th>{t("admin.pages.blend_listings.order")}</th>
                    <th>Actif</th>
                    <th>Action</th>
                  </tr>
                </thead>
                <tbody>
                  {listings.map((listing) => (<tr key={listing.id}>
                      ...
                      <td>
                        <span>{(listing.blend?.ingredients || []).length}</span>
                      </td>
                      <td>
                      ...
                      <td>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <button type="button" className="admin-icon-button admin-btn-secondary" onClick={() => {
                    if (editingId === listing.id && createDraft) {
                        ...
                        openEditForm(listing);
                ...
                }} disabled={Boolean(savingId)} aria-label={editingId === listing.id ? t("admin.pages.blend_listings.cancel_edit") : t("admin.pages.blend_listings.edit")} title={editingId === listing.id ? t("admin.pages.blend_listings.cancel_edit") : t("admin.pages.blend_listings.edit")}>
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                          ...
                          <button type="button" className="admin-icon-button admin-icon-button-danger" onClick={() => void deleteListing(listing)} disabled={Boolean(savingId)} aria-label={t("admin.pages.blend_listings.delete")} title={t("admin.pages.blend_listings.delete")}>
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
        ...
        {showCsvModal && (<div className="admin-modal-backdrop" onClick={closeCsvModal}>
            <div className="admin-modal" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 680 }}>
              ...
              <p className="admin-muted" style={{ marginBottom: '1rem' }}>{t("admin.pages.blend_listings.template_import_utilisent")}</p>

              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem', marginBottom: '1rem' }}>
                <button type="button" className="admin-btn admin-btn-secondary" onClick={downloadTemplateCsv}>
                  {t("admin.pages.blend_listings.download_modele_csv")}
                </button>
                <button type="button" className="admin-btn admin-btn-secondary" onClick={exportListingsCsv}>{t("admin.pages.blend_listings.exporter_blends")}</button>
              </div>

              <div className="admin-card" style={{ padding: '1rem' }}>
                <label className="admin-label" style={{ marginBottom: '0.5rem' }}>Importer un CSV</label>
                <input type="file" accept=".csv,text/csv" className="admin-input" onChange={(e) => {
                const nextFile = e.target.files?.[0] || null;
                setCsvFile(nextFile);
                setCsvImportError(null);
                setCsvImportSummary(null);
            }}/>
                {csvFile && (<p className="admin-muted" style={{ marginTop: '0.5rem' }}>{t("admin.pages.blend_listings.file_selected")}{csvFile.name}
                  </p>)}
                {csvImportError && (<div className="admin-alert" style={{ marginTop: '0.75rem' }}>
                    {csvImportError}
                  </div>)}
                {csvImportSummary && (<div className="admin-alert" style={{ marginTop: '0.75rem', whiteSpace: 'pre-line' }}>
                    {csvImportSummary}
                  </div>)}
              </div>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '0.5rem', marginTop: '1rem' }}>
                ...
                <button type="button" className="admin-btn admin-btn-primary" onClick={() => void importCsvFile()} disabled={!csvFile || importingCsv}>
                  {importingCsv ? 'Import en cours...' : t("admin.pages.blend_listings.importer_csv")}
                </button>
              </div>
            </div>
          </div>)}
