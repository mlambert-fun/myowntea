...
export default function Emails() {
    ...
    const summary = useMemo(() => metrics?.summary || {
        total: 0,
        sent: 0,
        failed: 0,
        retry: 0,
    ...
    async function loadAll() {
        try {
            ...
            showToast(err instanceof Error ? err.message : t("admin.pages.emails.failed_load_emails"), 'error');
        }
        finally {
            setLoading(false);
        }
    ...
    async function handleResend(id: string) {
        try {
            ...
            showToast(err instanceof Error ? err.message : t("admin.pages.emails.followup_failed"), 'error');
        }
        finally {
            setResendingId(null);
        }
    ...
    async function handleSendTest() {
        if (!testTo.trim()) {
            showToast(t("admin.pages.emails.renseigne_email_test"), 'error');
            return;
        }
        try {
            setSendingTest(true);
            ...
            showToast(err instanceof Error ? err.message : t("admin.pages.emails.failure_envoi_test"), 'error');
        }
        finally {
      ...
      <div className="admin-page admin-page-premium-lite">
        <div className="admin-header">
          <div>
            ...
            <p className="admin-subtitle">{t("admin.pages.emails.history_followup_kpi")}</p>
          </div>
          <button className="admin-btn admin-btn-secondary" onClick={() => void loadAll()} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
        ...
        <div className="admin-card" style={{ marginBottom: '1rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'nowrap', marginTop: '0.75rem' }}>
            <input className="admin-input" placeholder={t("admin.pages.emails.email_domaine_com")} value={testTo} onChange={(event) => setTestTo(event.target.value)} style={{ flex: 1, minWidth: 0 }}/>
            <button className="admin-btn admin-btn-primary" onClick={() => void handleSendTest()} disabled={sendingTest} style={{ flexShrink: 0 }}>
        ...
        <div className="admin-card" style={{ marginBottom: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap', marginTop: '0.75rem' }}>
            <select className="admin-input" value={statusFilter} onChange={(event) => {
            setPage(1);
            setStatusFilter(event.target.value);
        }}>
              <option value="">{t("admin.pages.emails.all_statuses")}</option>
              {statusOptions.map((status) => (<option key={status} value={status}>
                  {status}
                </option>))}
            </select>
            <select className="admin-input" value={typeFilter} onChange={(event) => {
            setPage(1);
            setTypeFilter(event.target.value);
        }}>
              <option value="">{t("admin.pages.emails.all_types")}</option>
              {typeOptions.map((type) => (<option key={type} value={type}>
                  {type}
                </option>))}
            </select>
            <input className="admin-input" placeholder="Destinataire" value={recipientFilter} onChange={(event) => {
            setPage(1);
        ...
        <div className="admin-card">
            ...
            <table className="admin-table">
              <thead>
                <tr>
                  ...
                  <th>Destinataire</th>
                  <th>Sujet</th>
                  <th>{t("admin.pages.emails.order")}</th>
                  <th>Statut</th>
                  <th>Tentatives</th>
                  <th>Erreur</th>
                  <th>Action</th>
              ...
              <tbody>
                {rows.map((row) => (<tr key={row.id}>
                    ...
                    <td>
                      <button className="admin-icon-button admin-btn-secondary" title="Renvoyer" aria-label={`Renvoyer l'email ${row.id}`} onClick={() => void handleResend(row.id)} disabled={resendingId === row.id}>
                        {resendingId === row.id ? ('...') : (<svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                            <path d="M4 12l7-7v4h4a5 5 0 0 1 0 10h-1" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                          </svg>)}
                      </button>
                    </td>
                  </tr>))}
                {!loading && rows.length === 0 && (<tr>
                    <td colSpan={9} style={{ textAlign: 'center' }} className="admin-muted">{t("admin.pages.emails.none_email_trouve")}</td>
                  </tr>)}
              </tbody>
            </table>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '1rem', gap: '0.75rem' }}>
            <button className="admin-btn admin-btn-secondary" disabled={page <= 1} onClick={() => setPage((prev) => Math.max(1, prev - 1))} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                <path d="M15 18l-6-6 6-6" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>{t("admin.pages.emails.prev")}</button>
            <div className="admin-muted">
              Page {page} / {totalPages}
            </div>
            <button className="admin-btn admin-btn-secondary" disabled={page >= totalPages} onClick={() => setPage((prev) => Math.min(totalPages, prev + 1))} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem' }}>
              Suivant
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
