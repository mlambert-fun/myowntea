...
export default function Customers() {
    ...
    const getCustomerLabel = (customer: Customer) => {
        ...
        return t("admin.pages.customers.guest");
    };
    async function confirmDeleteCustomer() {
        ...
        try {
            ...
            setDeleteError(t("admin.pages.customers.failed_delete_customer"));
        }
        finally {
            setIsDeleting(false);
        }
    ...
    const filteredCustomers = customers.filter((customer) => {
        ...
        if (filters.accountType === 'account' && !customer.email)
            return false;
        if (filters.accountType === 'guest' && customer.email)
            return false;
        ...
        if ((createdFrom || createdTo) && !createdAt)
            return false;
    ...
    return (<Layout>
      <div className="admin-page admin-page-premium-lite">
        ...
        {deleteError && (<div className="admin-alert admin-alert-error" style={{ marginBottom: '0.75rem' }}>
            {deleteError}
          </div>)}
        ...
        {showFilters && (<div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.75rem', marginBottom: '0.75rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', whiteSpace: 'nowrap' }}>
              ...
              <select className="admin-input" value={filters.accountType} onChange={(e) => setFilters({ ...filters, accountType: e.target.value })} style={{ width: '245px' }}>
                ...
                <option value="">{t("admin.pages.customers.all_types")}</option>
              </select>
            </div>
        ...
        <div className="admin-table-wrapper">
          <table className="admin-table">
            ...
            const typeLabel = customer.email ? t("admin.pages.customers.account") : t("admin.pages.customers.guest");
            return (<tr key={customer.id}>
                    <td>{typeLabel}</td>
                    <td>
                      {customer.email ? (<Link to={`/customers/${customer.id}`} className="admin-link-inline">
                          {customer.email}
                        </Link>) : ('-')}
                    </td>
                    <td>
                      <Link to={`/customers/${customer.id}`} className="admin-link-inline">
                        ...
                        {!customer.firstName && !customer.lastName ? t("admin.pages.customers.guest") : ''}
                      </Link>
                    </td>
              ...
              {filteredCustomers.length === 0 && (<tr>
                  <td colSpan={7} style={{ textAlign: 'center', color: 'var(--muted)' }}>{t("admin.pages.customers.none_customer_trouve")}</td>
                </tr>)}
            </tbody>
          </table>
        </div>
        ...
        {deleteTarget && (<div className="admin-modal-backdrop" onClick={() => !isDeleting && setDeleteTarget(null)}>
            <div className="admin-modal" onClick={(event) => event.stopPropagation()}>
            <h2 className="admin-card-title" style={{ fontSize: '1.25rem', marginBottom: '0.75rem' }}>{t("admin.pages.customers.delete_customer")}</h2>
            <p style={{ marginBottom: '1.5rem', color: 'var(--muted)' }}>{t("admin.pages.customers.etes_vous_vouloir")}<strong>{getCustomerLabel(deleteTarget)}</strong> ? {t("admin.pages.customers.action_irreversible_delete")}
            </p>
