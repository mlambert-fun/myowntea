      ...
      <div className="admin-page admin-page-premium-lite">
        ...
        <div className="admin-card">
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end' }}>
            <button className="admin-btn admin-btn-primary" type="submit">{t("admin.pages.options.add")}</button>
          </div>
        </form>
      </div>
          ...
          <div className="admin-grid-2">
            {options.map((option) => (<div key={option.id} className="admin-card" style={{ background: '#fff' }}>
                ...
                <div style={{ marginTop: '1rem' }}>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                    {option.values.map((value) => (<span key={value.id} className="admin-badge" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.35rem' }}>
                        {value.value}
                        <button className="admin-icon-button admin-icon-button-danger" style={{ width: '22px', height: '22px' }} onClick={() => handleDeleteValue(value.id)} type="button" title={t("admin.pages.options.delete")} aria-label={t("admin.pages.options.delete")}>×</button>
                      </span>))}
                  </div>
                ...
                [option.id]: e.target.value,
                    ...
                    <button className="admin-btn admin-btn-secondary" type="button" onClick={() => handleCreateValue(option.id)}>{t("admin.pages.options.add")}</button>
                  </div>
                </div>
              </div>))}
          </div>
