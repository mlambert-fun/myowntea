...
const discountSchema = discountBaseSchema.superRefine((data: DiscountFormState, ctx: z.RefinementCtx) => {
  ...
  if (advancedTypes.includes(data.type as (typeof advancedTypes)[number])) {
    if (!data.configJson || data.configJson.trim().length === 0) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Configuration JSON requise pour ce type', path: ['configJson'] });
    } else {
      try {
...
export default function DiscountFormPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setApiError('Impossible de charger la r?duction');
      } finally {
        setLoading(false);
      }
    };
  ...
  const onSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setApiError('Impossible d\'enregistrer la r?duction');
    } finally {
      setSaving(false);
    }
  };
    ...
    <Layout>
      <div className="admin-page admin-page-premium-lite">
        <div className="admin-header">
          <div>
            <h2 className="admin-title">{id ? 'Modifier la r?duction' : 'Nouvelle r?duction'}</h2>
            <p className="admin-subtitle">Configurez les r?gles de r?duction.</p>
          </div>
          <Link className="admin-btn admin-btn-secondary" to="/discounts">Retour</Link>
        </div>
            ...
            <form onSubmit={onSubmit} className="admin-form">
              <div className="admin-grid-2">
                ...
                <div>
                  ...
                  <input
                    ...
                    disabled={form.type !== 'PERCENTAGE' && form.type !== 'FIXED'}
                ...
                <div style={{ gridColumn: '1 / -1' }}>
                  ...
                  <textarea
                    className="admin-input"
                    style={{ minHeight: '140px', fontFamily: 'ui-monospace, SFMono-Regular, Menlo, monospace', fontSize: '12px' }}
                    value={form.configJson}
                    onChange={(e) => handleChange('configJson', e.target.value)}
                    placeholder={ADVANCED_CONFIG_PLACEHOLDER}
                  />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleChange('minSubtotal', e.target.value)}
                    placeholder="45.00"
                  />
                </div>
                  ...
                  <div className="admin-grid-2" style={{ marginTop: '0.75rem' }}>
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleChange('usageLimitTotal', e.target.value)}
                      />
              ...
              <div className="admin-section">
                ...
                <div className="admin-checkbox">
                  ...
                  />
                  <span>Premi?re commande uniquement</span>
                </div>
              </div>
