...
export default function Packs() {
    ...
    const componentProducts = useMemo(() => allProducts.filter((product) => product.type !== 'PACK' && product.type !== 'SUBSCRIPTION'), [allProducts]);
    const selectedPack = useMemo(() => packProducts.find((pack) => pack.id === selectedPackId) || null, [packProducts, selectedPackId]);
    const totalUnits = useMemo(() => packItems.reduce((sum, item) => sum + Number(item.qty || 0), 0), [packItems]);
    return (<Layout>
      <div className="admin-page admin-page-premium">
        <div className="admin-header">
          <div>
            <h1 className="admin-title">Packs</h1>
            <p className="admin-subtitle">{t("admin.pages.packs.composez_packs_precision")}</p>
          </div>
        </div>

        <section className="admin-premium-kpis">
          <article className="admin-premium-kpi-card">
            <p className="admin-muted">{t("admin.pages.packs.pack_selected")}</p>
          ...
          <article className="admin-premium-kpi-card">
            <p className="admin-muted">{t("admin.pages.packs.unites_totales")}</p>
        ...
        <div className="admin-card admin-premium-card">
          ...
          <p className="admin-muted admin-premium-card-subtitle">{t("admin.pages.packs.select_pack_then")}</p>

          <div className="admin-premium-form-grid">
            <div>
              <label className="admin-label">Pack</label>
              <select className="admin-input" value={selectedPackId} onChange={(e) => setSelectedPackId(e.target.value)}>
                <option value="">{t("admin.pages.packs.select_pack")}</option>
                {packProducts.map((pack) => (<option key={pack.id} value={pack.id}>
                    {pack.title}
                  </option>))}
              </select>
            </div>
            ...
            <div>
              ...
              <select className="admin-input" value={componentProductId} onChange={(e) => setComponentProductId(e.target.value)}>
                <option value="">{t("admin.pages.packs.select_product")}</option>
                {componentProducts.map((product) => (<option key={product.id} value={product.id}>
                    {product.title}
                  </option>))}
              </select>
            </div>
            ...
            <div>
              ...
              <select className="admin-input" value={componentVariantId} onChange={(e) => setComponentVariantId(e.target.value)}>
                <option value="">{t("admin.pages.packs.select_variant")}</option>
                {componentVariants.map((variant) => (<option key={variant.id} value={variant.id}>
            ...
            <div>
              <label className="admin-label">{t("admin.pages.packs.quantity")}</label>
              <input className="admin-input" type="number" min={1} value={qty} onChange={(e) => setQty(e.target.value)}/>
            </div>
          </div>

          <div className="admin-premium-actions">
            <button className="admin-btn admin-btn-primary" onClick={addPackItem} disabled={!selectedPackId || !componentVariantId}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                <path d="M12 5v14M5 12h14" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"/>
              </svg>{t("admin.pages.packs.add_pack")}</button>
          </div>
        </div>

        {selectedPackId && (<div className="admin-card admin-premium-card">
            <h2 className="admin-card-title">{t("admin.pages.packs.composants_pack")}</h2>
            <p className="admin-muted admin-premium-card-subtitle">
              {selectedPack?.title || t("admin.pages.packs.pack_selected")} • {packItems.length} composant(s)
            </p>

            {packItems.length === 0 ? (<div className="admin-empty-state">{t("admin.pages.packs.none_composant_pack")}</div>) : (<div style={{ overflowX: 'auto' }}>
                <table className="admin-table">
                  <thead>
                    <tr>
                      <th>Variante</th>
                      <th>{t("admin.pages.packs.quantity")}</th>
                      <th>Action</th>
                    </tr>
                  </thead>
                  <tbody>
                    {packItems.map((item) => (<tr key={item.id}>
                        <td>{item.componentVariant?.product?.title || item.componentVariantId}</td>
                        <td>
                          <input className="admin-input" type="number" min={1} value={item.qty} onChange={(e) => updatePackItem(item.id, Number(e.target.value))} style={{ width: '110px' }}/>
                        </td>
                        <td>
                          <button className="admin-icon-button admin-icon-button-danger" onClick={() => removePackItem(item.id)} title={t("admin.pages.packs.delete")} aria-label={t("admin.pages.packs.delete")}>
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
