  ...
  return (
    <Layout>
      <div className="admin-page admin-page-premium-lite">
        ...
        <div className="admin-card">
          <h2 className="admin-card-title">{editingId ? 'Modifier la variante' : 'Nouvelle variante'}</h2>
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginTop: '0.75rem' }}>
                    {option.values.map((value) => (
                      <label key={value.id} className="admin-chip">
                        <input
                          type="checkbox"
                          checked={formData.optionValueIds.includes(value.id)}
                ...
                {variants.map((variant) => (
                  <tr key={variant.id}>
                    ...
                    <td style={{ display: 'flex', gap: '0.5rem' }}>
                      <button className="admin-btn admin-btn-secondary" onClick={() => startEdit(variant)}>
                        Modifier
                      </button>
                      <button
