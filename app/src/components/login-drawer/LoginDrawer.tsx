      ...
      <aside className={`absolute right-0 top-0 h-full w-full sm:w-[480px] bg-white shadow-2xl transition-transform duration-300 ${open ? 'translate-x-0' : 'translate-x-full'}`} onClick={(event) => event.stopPropagation()}>
        <div className="flex h-full flex-col">
          <div className="flex items-center justify-between px-6 py-5 border-b border-[#EEE6D8]">
            <div>
              <h3 className="font-display text-xl text-[var(--sage-deep)]">{t("app.components.login_drawer.login_drawer.login")}</h3>
              <p className="text-xs text-[var(--sage-deep)]/60">{t("app.components.login_drawer.login_drawer.accedez_espace_customer")}</p>
            </div>
            <button onClick={onClose} className="rounded-full p-2 text-[var(--sage-deep)]/70 hover:text-[var(--sage-deep)] hover:bg-[#F3F1EE] transition" aria-label={t("app.components.login_drawer.login_drawer.close")}>
              <X className="w-4 h-4"/>
            </button>
          </div>
          <div className="flex-1 overflow-y-auto px-6 py-6">
            ...
            try {
              ...
              <input type="email" autoComplete="email" required placeholder={t("app.components.login_drawer.login_drawer.email_2")} value={email} onChange={(e) => setEmail(e.target.value)} className="input-elegant w-full"/>
              <input type="password" required placeholder={t("app.components.login_drawer.login_drawer.password")} value={password} onChange={(e) => setPassword(e.target.value)} className="input-elegant w-full"/>
              <div className="flex justify-end">
                <button type="button" className="text-xs text-[var(--sage-deep)]/70 underline hover:text-[var(--gold-antique)]" onClick={() => {
            onClose();
            window.location.href = '/forgot-password';
        }}>{t("app.components.login_drawer.login_drawer.password_oublie")}</button>
              </div>
              <button className="w-full btn-primary" disabled={loading}>
                {loading ? t("app.components.login_drawer.login_drawer.login_2") : t("app.components.login_drawer.login_drawer.login_cta")}
              </button>
            </form>
            ...
            {!customer?.email && (<div className="mt-4">
                ...
                window.location.href = `${apiBaseUrl}/auth/google/start`;
                  ...
                  </span>{t("app.components.login_drawer.login_drawer.continue_with_google")}</button>
              </div>)}

            <div className="text-xs text-[var(--sage-deep)]/60 mt-4">{t("app.components.login_drawer.login_drawer.pas_encore_account")}{' '}
              <button className="underline" onClick={() => (window.location.href = '/register')}>{t("app.components.login_drawer.login_drawer.create_account")}</button>
            </div>
          </div>
        </div>
      </aside>
