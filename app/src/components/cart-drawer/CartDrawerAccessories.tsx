...
export function CartDrawerAccessories() {
    ...
    const title = useMemo(() => t("app.components.cart_drawer.cart_drawer_accessories.accessoires_uniques_accompagner"), []);
    if (!loading && !hasItems) {
        return null;
    }
    return (<div className="space-y-3">
      ...
      {loading ? (<InlineLoading label={t("app.components.cart_drawer.cart_drawer_accessories.loading_accessoires")} textClassName="text-xs text-[var(--sage-deep)]/60"/>) : (<div className="flex gap-4 overflow-x-auto pb-2 snap-x snap-mandatory">
          {items.map((item) => (<div key={item.id} className="min-w-[180px] max-w-[180px] snap-start rounded-2xl border border-[#EEE6D8] bg-white p-3 flex flex-col">
              <div className="h-44 w-full overflow-hidden rounded-xl bg-[#F3F1EE] mb-3 cursor-pointer" onClick={() => {
                    window.location.href = `/accessoires/${item.slug}`;
                }}>
                <img src={item.image} alt={item.name} className="h-full w-full object-cover"/>
              </div>
              <div className="flex-1">
                <div className="text-sm font-medium text-[var(--sage-deep)] line-clamp-2 cursor-pointer" onClick={() => {
                    window.location.href = `/accessoires/${item.slug}`;
                }}>
                  {item.name}
                </div>
              ...
              <button className="mt-3 w-full btn-secondary text-xs py-2" onClick={() => addVariantToCart({
                ...
                })}>{t("app.components.cart_drawer.cart_drawer_accessories.add")}</button>
            </div>))}
        </div>)}
    </div>);
}
