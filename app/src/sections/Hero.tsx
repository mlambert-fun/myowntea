...
export function Hero({ ingredientCountLabel = '40+' }: { ingredientCountLabel?: string }) {
    ...
    return (<section ref={heroRef} className="relative min-h-screen w-full overflow-hidden bg-gradient-to-b from-[#F5F1E8] to-[#FAF8F3]">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="parallax absolute top-[50%] right-[25%] w-2 h-2 rounded-full bg-[var(--gold-antique)] opacity-40 animation-pulse-soft" style={{ animationDelay: '1s' }}/>
        <div className="parallax absolute top-[75%] right-[30%] w-4 h-4 rounded-full bg-[var(--gold-antique)] opacity-25 animation-pulse-soft" style={{ animationDelay: '0.5s' }}/>
      </div>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-6 lg:px-12">
        <div className="max-w-4xl mx-auto text-center">
          {/* Badge */}
          <div className="inline-flex items-center gap-2 px-4 py-2 mb-8 rounded-full bg-white/60 backdrop-blur-sm border border-[var(--gold-antique)]/30 animate-slide-up" style={{ animationDelay: '0.1s' }}>
            <Sparkles className="w-4 h-4 text-[var(--gold-antique)]"/>
            <span className="text-sm font-medium text-[var(--sage-deep)] tracking-wide">{t("app.sections.hero.art_melange")}</span>
          </div>

          {/* Main Title */}
          <h1 className="font-display text-5xl md:text-6xl lg:text-7xl font-medium text-[var(--sage-deep)] mb-6 leading-tight animate-slide-up" style={{ animationDelay: '0.2s' }}>{t("app.sections.hero.composez")}<span className="block italic text-[var(--gold-antique)] mt-2">{t("app.sections.hero.tea_unique")}</span>
          </h1>

          {/* Subtitle */}
          <p className="text-lg md:text-xl text-[var(--sage-deep)]/70 max-w-2xl mx-auto mb-10 leading-relaxed animate-slide-up" style={{ animationDelay: '0.4s' }}>{t("app.sections.hero.devenez_architecte_flavors")}</p>

          {/* CTA Button */}
          <div className="animate-slide-up" style={{ animationDelay: '0.6s' }}>
            <button onClick={scrollToCreator} className="btn-primary inline-flex items-center gap-3 group px-6">
              <span>{t("app.sections.hero.commencer_my_blend")}</span>
            ...
            <div className="w-full max-w-4xl mx-auto">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 md:gap-16">
                <div className="text-center">
                  ...
                  <div className="text-sm text-[var(--sage-deep)]/60 mt-1">{t("app.sections.hero.ingredients")}</div>
                </div>
                <div className="text-center">
              ...
              <div className="mt-6 pt-6 border-t border-[var(--sage-deep)]/15">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 md:gap-16">
                  <div className="text-center">
                    ...
                    <div className="text-sm text-[var(--sage-deep)]/60">{t("app.sections.hero.shipping_offerte")} {freeShippingThresholdLabel} €</div>
                  </div>
                  <div className="text-center">
                    <ShieldCheck className="w-6 h-6 text-[var(--gold-antique)] mx-auto mb-2"/>
                    <div className="text-sm text-[var(--sage-deep)]/60">{t("app.sections.hero.secure_payment")}</div>
                  </div>
                  <div className="text-center">
                    <Gift className="w-6 h-6 text-[var(--gold-antique)] mx-auto mb-2"/>
                    <div className="text-sm text-[var(--sage-deep)]/60">{t("app.sections.hero.echantillons_gratuits")}</div>
                  </div>
                </div>
              </div>
            </div>
