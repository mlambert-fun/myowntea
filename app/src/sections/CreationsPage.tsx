...
export default function CreationsPage() {
    ...
    useEffect(() => {
        ...
        api.getBlendListings()
            ...
            if (!mounted)
                return;
            setListings(Array.isArray(data)
                ? [...data].sort((left, right) => {
                    const leftRanking = Number.isFinite(left?.ranking) ? left.ranking : 0;
                    const rightRanking = Number.isFinite(right?.ranking) ? right.ranking : 0;
    ...
    const toCreatorIngredient = (entry: any): CreatorIngredient | null => {
        ...
        return {
            id: String(id),
            name: String(ingredient?.name || entry?.name || t("app.sections.creations_page.ingredient_2")),
      ...
      <section className="min-h-screen bg-[#FAF8F3] pt-28 pb-16">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="flex flex-col gap-2 mb-10">
            <h1 className="font-display text-4xl text-[var(--sage-deep)]">{t("app.sections.creations_page.blends_community")}</h1>
            <p className="text-[var(--sage-deep)]/70 max-w-2xl">{t("app.sections.creations_page.decouvrez_melanges_inspirants")}</p>
          </div>

          {loading ? (<DataLoadingState size="md" className="py-8" title={t("app.sections.creations_page.loading_blends")} titleClassName="text-[var(--sage-deep)]/70"/>) : listings.length === 0 ? (<div className="bg-white rounded-2xl p-6 text-[var(--sage-deep)]/70">{t("app.sections.creations_page.none_blend_available")}</div>) : (<div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {listings.map((listing) => {
                ...
                const creatorInfo = createdByLabel
                    ? t("app.sections.creations_page.created_by", undefined, { name: createdByLabel })
                    : orderCreatorLabel
                        ? t("app.sections.creations_page.created_by", undefined, { name: orderCreatorLabel })
                        : '';
                return (<div key={listing.id} className="group bg-white rounded-2xl shadow overflow-hidden cursor-pointer" onClick={() => goToCreation(listing.slug)}>
                    <div className="relative h-[22rem] overflow-hidden bg-[var(--cream-apothecary)] flex items-center justify-center">
                      ...
                      <img src={coverImageUrl || '/assets/misc/ingredient_placeholder.png'} alt={listing.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"/>
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                        ...
                        goToCreation(listing.slug);
                            ...
                            <Info className="w-4 h-4"/>{t("app.sections.creations_page.view_details")}</button>
                        </div>
                      </div>
                    </div>
                    <div className="p-5">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <span className="font-display text-[var(--gold-antique)] text-lg">
                      ...
                      {orderedIngredients.length > 0 && (<div className="mt-3 grid grid-cols-4 gap-2">
                          {orderedIngredients.map((entry: any, index: number) => {
                            const ingredient = entry.ingredient || entry;
                            const ingredientName = ingredient?.name || entry.name || t("app.sections.creations_page.ingredient");
                      ...
                      <div className="mt-4 flex items-center gap-3">
                        <button className="btn-secondary" style={{ paddingLeft: '0.7rem', paddingRight: '0.7rem' }} onClick={(e) => {
                        e.stopPropagation();
                        clearBlend();
                        orderedIngredients
                        ...
                        navigate('/?a=creator');
                    }}>{t("app.sections.creations_page.edit_atelier")}</button>
                        <button className="btn-primary" style={{ paddingLeft: '1rem', paddingRight: '1rem' }} onClick={(e) => {
                        e.stopPropagation();
                        ...
                        setSelectedBlendFormat(DEFAULT_BLEND_FORMAT);
                    }}>{t("app.sections.creations_page.add_cart")}</button>
                      </div>
                    </div>
                  </div>);
            })}
        ...
        {pendingBlendCartItem && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm cursor-close-cross" onClick={closeBlendFormatModal}>
            <div className="bg-white rounded-3xl max-w-xl w-full p-6 cursor-default" onClick={(event) => event.stopPropagation()}>
              <h3 className="font-display text-2xl text-[var(--sage-deep)] mb-5 text-center">{t("app.sections.creations_page.choisissez_format")}</h3>
              ...
              <BlendPurchaseSelector
                ...
                blendFormat={selectedBlendFormat}
                basePriceCents={pendingBlendCartItem.priceByFormatCents?.[selectedBlendFormat] ?? Math.max(0, Math.round(pendingBlendCartItem.price * 100))}
