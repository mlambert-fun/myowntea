...
export interface CartItem {
  ...
  isGift?: boolean;
  name: string;
  ingredients?: Array<{ name: string; ingredientColor: string; category?: string }>;
  ...
  packItems?: Array<{ variantId: string; title: string; qty: number; imageUrl?: string | null }>;
  price: number;
  quantity: number;
  color?: string;
}
...
interface BlendContextType {
  ...
  /* Cart API */
  cartItems: CartItem[];
  addToCart: (item: Omit<CartItem, 'id' | 'quantity' | 'itemType'> & { quantity?: number }) => void;
  addVariantToCart: (item: { variantId?: string; productId?: string; name: string; priceCents: number; imageUrl?: string | null; quantity?: number; selectedOptions?: Array<{ name: string; value: string }> }) => void;
...
export function BlendProvider({ children }: { children: React.ReactNode }) {
  ...
  const [cartItems, setCartItems] = useState<CartItem[]>(() => {
    try {
      ...
      const meta = metaRaw ? (JSON.parse(metaRaw) as { version?: string }) : null;
  ...
  const [appliedDiscountCode, setAppliedDiscountCode] = useState<string | null>(() => {
    try {
      ...
      if (raw) {
        const parsed = JSON.parse(raw) as { appliedDiscountCode?: string | null; version?: string };
  ...
  const mapCartResponse = useCallback((cart: CartResponse) => {
    ...
    const mappedItems: CartItem[] = cart.items.map((item) => {
      ...
      return {
        ...
        itemType: 'BLEND',
        isGift: Boolean(item.isGift || item.snapshot?.isGift),
        name: item.snapshot?.title || 'Mon mélange',
    ...
    const orderedItems = mappedItems
      ...
      .sort((a, b) => {
        ...
        if (aIndex === undefined && bIndex === undefined) return a.index - b.index;
  ...
  const totalPrice = (() => {
    ...
    const totals = selectedIngredients.reduce(
      (acc, ingredient) => {
        const priceCents = Math.max(
          0,
          Math.round((typeof ingredient.basePrice === 'number' && Number.isFinite(ingredient.basePrice) ? ingredient.basePrice : 0) * 100)
        );
        const isBase = String(ingredient.category || '').toLowerCase() === 'base';
        if (isBase) {
          acc.baseCount += 1;
          acc.baseTotal += priceCents;
        } else {
          acc.nonBaseTotal += priceCents;
        }
        return acc;
      },
    ...
    if (totals.baseCount > 0) {
      ...
      totalPriceCents = Math.round(totals.nonBaseTotal + averagedBaseTotal);
    } else {
      ...
      totalPriceCents = Math.round(clampedNoBaseCents);
    }
  ...
  const applyBlendOptimistic = useCallback((item: Omit<CartItem, 'id' | 'quantity' | 'itemType'> & { quantity?: number }) => {
    setCartItems((prev) => {
      const matchIndex = prev.findIndex((ci) => {
        if (ci.itemType !== 'BLEND') return false;
        if (ci.name !== item.name) return false;
        if (
          normalizeBlendFormat(ci.blendFormat || DEFAULT_BLEND_FORMAT) !==
          normalizeBlendFormat(item.blendFormat || DEFAULT_BLEND_FORMAT)
        ) {
          return false;
        }
        if ((ci.ingredientIds || []).length !== (item.ingredientIds || []).length) return false;
  ...
  const applyVariantOptimistic = useCallback((item: { variantId?: string; productId?: string; name: string; priceCents: number; imageUrl?: string | null; quantity?: number; selectedOptions?: Array<{ name: string; value: string }> }) => {
    setCartItems((prev) => {
      const matchIndex = prev.findIndex((ci) =>
        ci.itemType === 'VARIANT' && (
          (item.variantId && ci.variantId === item.variantId) ||
  ...
  const addToCart = useCallback((item: Omit<CartItem, 'id' | 'quantity' | 'itemType'> & { quantity?: number }) => {
  ...
  const addVariantToCart = useCallback((item: { variantId?: string; productId?: string; name: string; priceCents: number; imageUrl?: string | null; quantity?: number; selectedOptions?: Array<{ name: string; value: string }> }) => {
  ...
  const clearCartItems = useCallback(() => {
    ...
    try { localStorage.removeItem('tea_cart'); } catch (e) {}
    setCartItems([]);
  }, [isLoggedIn]);
  ...
  const refreshCartSummary = useCallback(async () => {
    ...
    const mapCartResponseToSummaryItems = (cart: CartResponse) =>
      cart.items.map((item) => ({
        ...
        variantId:
          typeof item.snapshot?.variantId === 'string'
    ...
    const mapLocalItemsToSummaryItems = (items: CartItem[]) =>
      items.map((item) => ({
    ...
    if (isLoggedIn) {
      ...
      try {
        ...
        if (!cart.items || cart.items.length === 0) {
          setCartSummary(null);
          setCartMessages([]);
          if (isCurrentRequest()) setIsCartSummaryLoading(false);
          return;
        }
        ...
        if (summaryItems.length === 0) {
          setCartSummary(null);
          setCartMessages([resolveErrorMessage(error, 'Impossible de recuperer le panier.')]);
          if (isCurrentRequest()) setIsCartSummaryLoading(false);
          return;
        }

        const localSubtotalCents = summaryItems.reduce(
          (sum, item) => sum + Math.max(0, Math.round(item.unitPriceCents || 0)) * Math.max(1, item.quantity || 1),
          0
        );
      ...
      try {
        ...
        setCartMessages([resolveErrorMessage(error, 'Impossible de calculer les réductions.')]);
      } finally {
        if (isCurrentRequest()) {
    ...
    try {
      ...
      if (!isCurrentRequest()) return;
      const shippingKey = `${shippingSelection?.mode || ''}|${shippingSelection?.offerId || ''}|${shippingSelection?.offerCode || ''}`;
      ...
      if (!isCurrentRequest()) return;
      const localSubtotalCents = cartItems.reduce(
        (sum, item) => sum + Math.max(0, Math.round(item.price * 100)) * Math.max(1, item.quantity || 1),
        0
      );
      setCartSummary(
  ...
  useEffect(() => {
    ...
    void api.getCart(getStoredShippingSelection())
      .then((cart) => mapCartResponse(cart))
      .catch(() => {
        setCartMessages(['Impossible de récupérer le panier.']);
      });
  ...
  const addCurrentBlendToWishlist = useCallback(async (options?: {
    ...
    const ingredientIds = selectedIngredients.map((ingredient) => ingredient.id).filter(Boolean);
    if (ingredientIds.length === 0) {
      return false;
    }
    ...
    const creationName = requestedName || blendName.trim() || 'Ma création';
    ...
    try {
      ...
      const successMessage = 'Votre création a bien été ajoutée à votre wishlist.';
      ...
      const errorMessage = 'Impossible d\'ajouter cette création à votre wishlist.';
  ...
  }, [blendName, isAccountLoggedIn, openWishlistDrawer, selectedIngredients]);
